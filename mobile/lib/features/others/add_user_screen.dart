...
class _AddUserScreenState extends ConsumerState<AddUserScreen> {
  ...
  final _empIdCtrl = TextEditingController();
  final _passwordCtrl =
      TextEditingController(text: 'user123'); // Default password
  String _role = 'STAFF'; // Default Role (Web uses Default)
  bool _isLoading = false;
          ...
          child: ListView(
            children: [
              ...
              ElevatedButton(
                ...
                child: _isLoading
                    ? const CircularProgressIndicator(color: Colors.white)
                    : const Text('建立用戶', style: TextStyle(fontSize: 18)),
              ),
            ],
          ),
        ),
